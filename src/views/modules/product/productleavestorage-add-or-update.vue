<template>
  <el-dialog
    ...
    <el-form-item label="0为正常，1为禁用" prop="status">
      <el-input v-model="dataForm.status" placeholder="0为正常，1为禁用"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '0为正常，1为禁用不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/product/productleavestorage/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/product/productleavestorage/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
